...
[[nodiscard]] CyclicKVCacheLayout plan_cyclic_kv_cache(LayoutBuilder& builder, std::uint32_t layers,
...
public:
    ...
    // Copies the complete fixed state. Source and destination must have identical layouts.
    void copy_from(const CyclicKVCache& source, cudaStream_t stream);
