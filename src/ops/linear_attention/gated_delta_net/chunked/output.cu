...
cudaError_t launch_fixed(const chunk_output_config& cfg, dim3 grid, head_map qk_map) {
    constexpr int smem_bytes = kernel::kernel_dims::SMEM_FLOATS * static_cast<int>(sizeof(float));

    cudaError_t err = cudaFuncSetAttribute(kernel::output_kernel,
                                           cudaFuncAttributeMaxDynamicSharedMemorySize, smem_bytes);
    if (err != cudaSuccess) { return err; }

    const dim3 block(kernel::THREADS, 1, 1);

    kernel::output_kernel<<<grid, block, smem_bytes, cfg.stream>>>(
...
cudaError_t launch_output(const chunk_output_config& cfg) {
    ...
    return launch_fixed(cfg, grid, qk_map);
}
