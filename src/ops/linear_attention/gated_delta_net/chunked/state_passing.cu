...
cudaError_t launch_fixed(const state_passing_config& cfg, head_map qk_map, int NT) {
    using D                  = kernel::kernel_dims;
    constexpr int smem_bytes = kernel::smem_layout::SMEM_FLOATS * static_cast<int>(sizeof(float));

    cudaError_t err = cudaFuncSetAttribute(kernel::state_passing_kernel,
    ...
    kernel::state_passing_kernel<<<grid, block, smem_bytes, cfg.stream>>>(
...
cudaError_t launch_state_passing(const state_passing_config& cfg) {
    ...
    NINFER_GATED_DELTA_NET_PROPAGATE(
        v.check_grid(static_cast<std::int64_t>(cfg.H_v) * kernel::kernel_dims::D_STRIPS, 1));
    return launch_fixed(cfg, qk_map, static_cast<int>(NT));
}
