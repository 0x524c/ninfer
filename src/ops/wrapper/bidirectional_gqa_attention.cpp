...
std::uint32_t validate_context(const PagedKVLayerView& context, const char* op) {
    ...
    require_shape(context.block_table, context.block_table.ne[0], 1, 1, 1, op,
                  "context block table");
    if (context.block_table.dtype != DType::I32) {
        throw std::invalid_argument(std::string(op) + ": context block table must be I32");
    }
    require_contiguous_nonnull(context.block_table, op, "context block table");
    ...
    const std::uint64_t logical_capacity =
        static_cast<std::uint64_t>(context.block_table.ne[0]) * kPagedKVPageSize;
