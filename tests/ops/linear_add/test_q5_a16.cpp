...
int q5_a16_conformance() {
    ...
    constexpr std::array<std::int32_t, 3> kRouteStarts{2, 25, 129};
    constexpr std::array<std::int32_t, 4> kRouteInteriors{1, 13, 64, 256};

    int failures = 0;
    failures += ninfer::test::linear_add::run_shape(
        "Q5_A16 LinearAdd", WeightFormat::Q5G64F16S,
        ShapeCase{5120, 6144, 401U, kRouteStarts, kRouteInteriors});
    failures += ninfer::test::linear_add::run_shape(
        "Q5_A16 LinearAdd", WeightFormat::Q5G64F16S,
        ShapeCase{5120, 17408, 409U, kRouteStarts, kRouteInteriors});
    return failures;
}
